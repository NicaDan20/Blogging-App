...
var RecaptchaV3 = (function () {
    ...
    RecaptchaV3.prototype.renderWith = function (optionsToOverride) {
        ...
        return '<script src="https://' + recaptcha_host + this._api.script + query_string + '"></script>' +
    ...
    RecaptchaV3.prototype.verify = function (req, cb) {
        ...
        var recaptcha_host = this._api.host;
        console.log("This host is " + recaptcha_host)
        if (this._options.useRecaptchaDomain)
            recaptcha_host = this._api.alt_host;
        if (!req)
            throw new Error('req is required');
        if (req.body && req.body['g-recaptcha-response']) {
            response = req.body['g-recaptcha-response'];
            console.log("This response is from req.body and it is is " + response)
        }
        if (req.query && req.query['g-recaptcha-response']) {
            response = req.query['g-recaptcha-response'];
            console.log("This response is from req.query and it is is " + response)
        }
        if (req.params && req.params['g-recaptcha-response']) {
            response = req.params['g-recaptcha-response'];
            console.log("This response is from req.params and it is is " + response)
        }
        else {
